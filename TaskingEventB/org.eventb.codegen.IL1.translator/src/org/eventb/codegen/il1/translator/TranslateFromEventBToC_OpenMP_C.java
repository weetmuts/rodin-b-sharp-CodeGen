...
public class TranslateFromEventBToC_OpenMP_C extends AbstractTranslateEventBToTarget {
	...
	protected void saveToFile(ArrayList<String> codeToSave,
		...
		for (int lineNumber = 0; lineNumber < codeToSave.size(); lineNumber++)			
		{
			...
			if (line.equals("// Protected"))
			{
				...
				IL1CodeFiler.getDefault().save(protectedCode,directoryName, name + ".c", translationManager);
			}
			else if (line.equals("// Task"))
			{
				...
				IL1CodeFiler.getDefault().save(taskCode,directoryName, name + ".c", translationManager);
			}
...
//				commonCode.add(codeGenerateTimestamp);
			...
			IL1CodeFiler.getDefault().save(commonCode, directoryName, "Common.c", translationManager);				
		}
