...
public class TranslationBinder {
	 ...
	 */
	public static String bind(String pattern, IBinding binding, Map<FreeIdentifier, String> typingMappings) {
		assert binding!=null;
	 ...
	 * @return the resultant string
	 */
	public static String bind(String pattern, IBinding binding) {
		assert binding!=null;
		patternSb = new StringBuilder(pattern);
	 ...
	 * @return the resultant string
	 */
	public static String bindAssignment(String pattern, IBinding binding, FreeIdentifier assignedIdentifier) {
		assert binding!=null;
		patternSb = new StringBuilder(pattern);
	 ...
	 * @return the resultant string
	 */
	public static String bindAssignment(String pattern, IBinding binding, FreeIdentifier assignedIdentifier, Map<FreeIdentifier, String> typingMappings) {
		assert binding!=null;
		patternSb = new StringBuilder(pattern);
...
//		Pattern p = Pattern.compile("[\\W&&\\D&&\\S|(>|<)]{1}");
	...
	private static String replaceAllPrimeIdentifier(String predicateInput, FreeIdentifier freeIdentifier, FormulaFactory ff){
		...
		output = regexReplace(freeIdentifier.getName(), output , freeIdentifier.withPrime(ff).toString());
...
//		new Random(i);
	 ...
	 */
	private static TreeMap<String,String> buildMapSubs(IBinding binding, Map<FreeIdentifier, String> typingMappings, FreeIdentifier assignedIdentifier){
		TreeMap<String,String> mapSubs = new TreeMap<String, String>();
		int regexIndex = 0;

		mapSubs.putAll(buildMapSubsExpression(binding.getExpressionMappings(), regexIndex, typingMappings, assignedIdentifier,binding.getFormulaFactory()));
		mapSubs.putAll(buildMapSubsPredicate(binding.getPredicateMappings(), mapSubs.size()));
...
//				patternSb = regexReplace(identifier.getName(), patternSb, regex);
			...
			if(assignedIdentifier!=null && expressionTranslated.equals(assignedIdentifier.withPrime(ff))){
				patternSb = new StringBuilder(replaceAllPrimeIdentifier(patternSb.toString(), identifier.withoutPrime(ff), ff));
